...
PlyFaceList& PlyFaceList::operator<<(string const& str_vertex)
{
...
PlyFaceList& PlyFaceList::operator<<(fstream& file)
{
...
PlyFaceList& PlyFaceList::operator<<(shared_ptr<PlyFace> const& vertex)
{
    this->faces->emplace_back(vertex);
    return *this;
}

PlyFaceList& PlyFaceList::operator<<(PlyFileEncoding const& fe)
{
    this->file_encoding = fe.GetDefinition();
    return *this;
}

bool PlyFaceList::read_element_face_names(fstream& file)
{
    string list_name;
    string list_type;
    string vertex_type;
...
void PlyFaceList::SetCountInHeader(unsigned int count)
{
    this->count_in_header = count;
}

unsigned PlyFaceList::GetCountInHeader() const
{
    return this->count_in_header;
}

std::shared_ptr<std::vector<std::shared_ptr<PlyFace>>> PlyFaceList::GetFaces() const
{
    return this->faces;
}
#endif
